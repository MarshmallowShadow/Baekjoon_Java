...
public class Nain {
	...
	static int maxSeats(String[] classroom) {
		...
		for(int i=0; i<classroom.length; i+=2) {
			for(int j=0; j<classroom[i].length(); j++) {
				if(classroom[i].charAt(j) == '.') count1++;
			}
		}
		for(int i=1; i<classroom.length; i+=2) {
			for(int j=0; j<classroom[i].length(); j++) {
				if(classroom[i].charAt(j) == '.') count2++;
			}
		}
