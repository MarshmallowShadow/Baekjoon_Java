...
public class Nain2 {
	public static void main(String[] args) throws IOException {
		...
		for(int i=0; i<C; i++) {
			...
			for(int j=0; j<N; j++) {
				//Each Column (x)
				for(int k=0; k<M; k++) {
					//Search for Seat Groups
					if(seats[j][k] != 0) {
						dp.add(new int[M]);
						search(j, k, seats, dp.get(dp.size()-1));
					}
				}
			}
			...
			for(int j=0; j<dp.size(); j++) {
				...
				for(int k=0; k<M; k++) {
					if(k == 2) {
						dp.get(j)[k] += dp.get(j)[k-2];
					} else if(k > 2) {
						if (dp.get(j)[k-2] > dp.get(j)[k-3]) {
							dp.get(j)[k] += dp.get(j)[k-2];
						} else {
							dp.get(j)[k] += dp.get(j)[k-3];
						}
					}
					
					if(dp.get(j)[k] > maxGroup) {
						maxGroup = dp.get(j)[k];
					}
				}
				maxCount += maxGroup;
	...
	public static void search(int posY, int posX, int[][] seats, int[] dp) {
		if(posX < 0 || posY < 0 || posY >= seats.length || posX >= seats[0].length) return;
		if(seats[posY][posX] == 0) return;
		else {
			dp[posX]++;
			seats[posY][posX] = 0;
			for(int i=-1; i<=1; i++) {
				search(posY+i, posX+1, seats, dp);
				search(posY+i, posX-1, seats, dp);
			}
		}
	}
